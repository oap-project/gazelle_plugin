...
## Memory allocation
The memory usage in Native SQL engine is high. The allocations goes to two parts: 1) Java based allocation which is widely used in Arrow Java API. 2) Native side memory allocation used in each native kernel. We made more progress here, the memroy footprint is stable during a TPC-DS power run.
