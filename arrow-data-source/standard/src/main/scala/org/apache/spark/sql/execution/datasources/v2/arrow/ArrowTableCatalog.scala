...
import com.intel.oap.spark.sql.execution.datasources.v2.arrow.ArrowTable
 ...
 */
class ArrowTableCatalog
  extends TableCatalog with SupportsNamespaces with SQLConfHelper {
  import ArrowTableCatalog._
  import org.apache.spark.sql.connector.catalog.CatalogV2Implicits.NamespaceHelper

  override val defaultNamespace: Array[String] = Array("default")

  lazy val catalog = SparkSession.active.sessionState.catalog
  ...
  override def loadTable(ident: Identifier): Table = {
    ...
    ArrowTable(
      ...
      SparkSession.active,
      new CaseInsensitiveStringMap(catalogTable.storage.properties.asJava),
      Seq(catalogTable.location.toString),
      Some(catalogTable.schema),
      classOf[ArrowFileFormat])
  }
      ...
      ident.namespace match {
        ...
        case _ =>
          throw new NoSuchTableException(
            s"V2 session catalog requires a single-part namespace: " +
              s"${ident.asTableIdentifier.quotedString}")
      }
    }
  }
...
private[sql] object ArrowTableCatalog {
