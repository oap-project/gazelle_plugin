...
class GazellePluginConfig(conf: SQLConf) extends Logging {
  ...
  val enableColumnarCodegenSort: Boolean =
    conf
      .getConfString("spark.oap.sql.columnar.codegen.sort", "true")
  ...
  // using new hashmap in columnar shuffledhashjoin or not
  val enableColumnarShuffledHashJoinNewMap: Boolean =
    conf.getConfString("spark.oap.sql.columnar.shuffledhashjoinNewMap", "false").toBoolean && enableCpu
...
object GazellePluginConfig {
  ...
  def getSpillThreshold: Long = synchronized {
    if (ins == null) {
      -1
    } else {
      ins.sortSpillThreshold
    }
  }
