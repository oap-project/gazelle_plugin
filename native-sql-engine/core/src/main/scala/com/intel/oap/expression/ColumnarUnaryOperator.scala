...
// StringSplit
class ColumnarStringSplit(child: Expression, regex: Expression, limit: Expression, original: Expression)
    extends StringSplit(child: Expression, regex: Expression, limit: Expression)
        with ColumnarExpression
        with Logging {
  ...
  override def doColumnarCodeGen(args: java.lang.Object): (TreeNode, ArrowType) = {
    ...
    val resultType = new ArrowType.Bool()
    val funcNode =
      TreeBuilder.makeFunction("split_part", Lists.newArrayList(child_node, regex_node, limit_node), resultType)
    (funcNode, resultType)
  }
}
...
class ColumnarStringTrim(child: Expression, trimStr: Option[Expression], original: Expression)
  ...
  override def doColumnarCodeGen(args: java.lang.Object): (TreeNode, ArrowType) = {
    val (child_node, childType): (TreeNode, ArrowType) =
      child.asInstanceOf[ColumnarExpression].doColumnarCodeGen(args)
    val (trimStr_node, trimStrType): (TreeNode, ArrowType) =
      trimStr.getOrElse(Literal("")).asInstanceOf[ColumnarExpression].doColumnarCodeGen(args)

    val resultType = new ArrowType.Bool()
    val funcNode =
      TreeBuilder.makeFunction("btrim", Lists.newArrayList(child_node, trimStr_node), resultType)
    (funcNode, resultType)
  }
}
...
class ColumnarStringTrimLeft(child: Expression, trimStr: Option[Expression], original: Expression)
  ...
  override def doColumnarCodeGen(args: java.lang.Object): (TreeNode, ArrowType) = {
    ...
    val resultType = new ArrowType.Bool()
    val funcNode =
      TreeBuilder.makeFunction("ltrim", Lists.newArrayList(child_node, trimStr_node), resultType)
    (funcNode, resultType)
  }
}
...
class ColumnarStringTrimRight(child: Expression, trimStr: Option[Expression], original: Expression)
  ...
  override def doColumnarCodeGen(args: java.lang.Object): (TreeNode, ArrowType) = {
    ...
    val resultType = new ArrowType.Bool()
    val funcNode =
      TreeBuilder.makeFunction("rtrim", Lists.newArrayList(child_node, trimStr_node), resultType)
    (funcNode, resultType)
  }
}
...
object ColumnarUnaryOperator {
  ...
  def create(child: Expression, original: Expression): Expression = original match {
    ...
    case a: StringSplit =>
      new ColumnarStringSplit(child, a.regex, a.limit, a)
    case a: StringTrim =>
      new ColumnarStringTrim(child, a.trimStr, a)
    case a: StringTrimLeft =>
      new ColumnarStringTrimLeft(child, a.trimStr, a)
    case a: StringTrimRight =>
      new ColumnarStringTrimRight(child, a.trimStr, a)
