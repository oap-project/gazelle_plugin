...
import com.intel.oap.tpc.ds.Orc_TPCDSTableGen._
...
class Orc_TPCDSTableGen(val spark: SparkSession, scale: Double, path: String)
    extends Serializable with TableGen {

  def writeOrcTable(name: String, rows: List[Row]): Unit = {
    ...
    writeOrcTable(name, rows, schema, partitionBy)
  }

  private def writeOrcTable(tableName: String, rows: List[Row], schema: StructType,
  ...
  override def gen(): Unit = {
    ...
    Table.getBaseTables.forEach { t =>
      ...
      writeOrcTable(t.getName, p)
      if (t.hasChild) {
        writeOrcTable(t.getChild.getName, c)
      }
    }
  }
  ...
  override def createTables(): Unit = {
    ...
    files.foreach(file => {
      ...
      spark.catalog.createTable(file.getName, "arrow",
        Map("path" -> file.getAbsolutePath, "originalFormat" -> "orc"))
      try {
