...
class PlannerSuite extends SharedSparkSession with AdaptiveSparkPlanHelper {
  ...
  */
  ...
  ignore("SPARK-33399: aliases should be handled properly in HashPartitioning") {
    withSQLConf(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "-1") {
      withTempView("t1", "t2", "t3") {
        spark.range(10).repartition($"id").createTempView("t1")
        spark.range(20).repartition($"id").createTempView("t2")
  ...
  ignore("SPARK-33399: alias handling should happen properly for RangePartitioning") {
    withSQLConf(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "-1") {
  ...
  ignore("SPARK-33399: aliased should be handled properly " +
  ...
  ignore("SPARK-33399: alias handling should happen properly for SinglePartition") {
    withSQLConf(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "-1") {
  ...
  ignore("SPARK-33399: No extra exchanges in case of" +
  ...
  ignore("SPARK-33400: Normalization of sortOrder should take care of sameOrderExprs") {
    withSQLConf(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "-1") {
      withTempView("t1", "t2", "t3") {
        spark.range(10).repartition($"id").createTempView("t1")
        spark.range(20).repartition($"id").createTempView("t2")
