...
abstract class RemoveRedundantProjectsSuiteBase
  ...
  ignore("join with ordering requirement") {
  ...
  ignore("SPARK-33697: UnionExec should require column ordering") {
    withTable("t1", "t2") {
      spark.range(-10, 20)
        .selectExpr(
          "id",
  ...
  ignore("SPARK-33697: remove redundant projects under expand") {
    val query =
      """
