...
abstract class RemoveRedundantSortsSuiteBase
  ...
  ignore("remove redundant sorts with broadcast hash join") {
    withTempView("t1", "t2") {
  ...
  ignore("remove redundant sorts with sort merge join") {
    withTempView("t1", "t2") {
      spark.range(1000).select('id as "key").createOrReplaceTempView("t1")
      spark.range(1000).select('id as "key").createOrReplaceTempView("t2")
      val query = """
  ...
  ignore("cached sorted data doesn't need to be re-sorted") {
    withSQLConf(SQLConf.REMOVE_REDUNDANT_SORTS_ENABLED.key -> "true") {
  ...
  ignore("SPARK-33472: shuffled join with different left and right side partition numbers") {
    withTempView("t1", "t2") {
