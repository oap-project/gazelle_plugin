...
abstract class BucketedReadSuite extends QueryTest with SQLTestUtils with AdaptiveSparkPlanHelper {
  ...
  ignore("SPARK-32767 Bucket join should work if SHUFFLE_PARTITIONS larger than bucket number") {
    withSQLConf(
  ...
  ignore("bucket coalescing eliminates shuffle") {
    withSQLConf(
  ...
  ignore("bucket coalescing is not satisfied") {
