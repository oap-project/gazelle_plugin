...
TEST_F(BenchmarkArrowComputeJoin, JoinBenchmarkWithCondition) {
  ...
  auto n_probeArrays = TreeExprBuilder::MakeFunction(
      "conditionedProbeArraysInner",
      {n_left_key, n_right_key, greater_than_function}, indices_type);
  ...
  auto probeArrays_expr =
      TreeExprBuilder::MakeExpression(n_codegen_probe, f_indices);
  ...
  TIME_MICRO_OR_THROW(elapse_gen,
                      CreateCodeGenerator(left_schema, {probeArrays_expr},
                                          field_list, &expr_probe, true));
