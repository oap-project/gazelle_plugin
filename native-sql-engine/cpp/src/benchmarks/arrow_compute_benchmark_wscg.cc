...
class BenchmarkArrowComputeWSCG : public ::testing::Test {
 public:
  void SetUp() override {
    ...
    std::cout << "This Benchmark used file " << left_path << " and "
              << right_path
    ...
    ASSERT_OK_AND_ASSIGN(
        right_fs, arrow::fs::FileSystemFromUri(right_path, &right_file_name));
    ASSERT_OK_AND_ASSIGN(
        left_fs, arrow::fs::FileSystemFromUri(left_path, &left_file_name));
    ...
    ASSERT_NOT_OK(::parquet::arrow::FileReader::Make(
        ...
        &left_parquet_reader));
    ASSERT_NOT_OK(left_parquet_reader->GetRecordBatchReader(
        {0}, {0, 1, 2}, &left_record_batch_reader));

    ASSERT_NOT_OK(::parquet::arrow::FileReader::Make(
        pool, ::parquet::ParquetFileReader::Open(right_file), properties,
        &right_parquet_reader));
    ASSERT_NOT_OK(right_parquet_reader->GetRecordBatchReader(
        {0}, {0, 1}, &right_record_batch_reader));
...
TEST_F(BenchmarkArrowComputeWSCG, JoinBenchmark) {
  ...
  auto n_left = TreeExprBuilder::MakeFunction("codegen_left_schema",
                                              left_field_node_list, uint32());
  ...
  auto n_left_key = TreeExprBuilder::MakeFunction(
      "codegen_left_schema", {left_field_node_list[left_primary_key_index]},
      uint32());
  auto n_right_key = TreeExprBuilder::MakeFunction(
      "codegen_right_schema", {right_field_node_list[right_primary_key_index]},
      uint32());
  ...
  std::merge(left_field_list.begin(), left_field_list.end(),
             right_field_list.begin(), right_field_list.end(),
             field_list.begin());
  ...
  auto n_result =
      TreeExprBuilder::MakeFunction("result", result_node_list, uint32());
  ...
  ASSERT_NOT_OK(CreateCodeGenerator(schema_table_0, {hashRelation_expr}, {},
                                    &expr_build, true));
  ...
  std::cout << "Readed left table with " << num_batches << " batches."
            << std::endl;

  TIME_MICRO_OR_THROW(elapse_finish,
                      expr_build->finish(&build_result_iterator));
  TIME_MICRO_OR_THROW(elapse_finish,
                      expr_probe->finish(&probe_result_iterator_base));
  ...
  do {
    TIME_MICRO_OR_THROW(elapse_right_read, right_record_batch_reader->ReadNext(
                                               &right_record_batch));
    if (right_record_batch) {
      ...
      TIME_MICRO_OR_THROW(elapse_probe_process, probe_result_iterator->Process(
                                                    right_column_vector, &out));
      num_batches += 1;
      num_output_batches++;
      num_rows += out->num_rows();
    }
  } while (right_record_batch);
  std::cout << "Readed right table with " << num_batches << " batches."
            << std::endl;

  std::cout << "=========================================="
            << "\nBenchmarkArrowComputeWSCG processed " << num_batches
            << " batches"
            ...
            << "\nProbe and Shuffle took "
            << TIME_TO_STRING(elapse_probe_process) << "\n"
            << "===========================================" << std::endl;
}
...
TEST_F(BenchmarkArrowComputeWSCG, MultipleJoinBenchmark) {
  ...
  auto n_probeArrays_1 =
      TreeExprBuilder::MakeFunction("conditionedProbeArraysAnti",
                                    {n_left, n_right, n_left_key, n_right_key,
                                     n_result, n_hash_config, n_condition},
                                    uint32());
  auto n_child_probe_1 = TreeExprBuilder::MakeFunction(
      "child", {n_probeArrays_1, n_child_probe}, uint32());

  auto n_wscg = TreeExprBuilder::MakeFunction("wholestagecodegen",
                                              {n_child_probe_1}, uint32());
  auto probeArrays_expr = TreeExprBuilder::MakeExpression(n_wscg, f_res);
  ...
  do {
    ...
    if (left_record_batch) {
      TIME_MICRO_OR_THROW(
          elapse_eval,
          expr_build_0->evaluate(left_record_batch, &dummy_result_batches));
      TIME_MICRO_OR_THROW(
          elapse_eval,
          expr_build_1->evaluate(left_record_batch, &dummy_result_batches));
      num_batches += 1;
    }
  } while (left_record_batch);
  std::cout << "Readed left table with " << num_batches << " batches."
  ...
  TIME_MICRO_OR_THROW(elapse_finish,
                      expr_build_0->finish(&build_result_iterator_0));
  TIME_MICRO_OR_THROW(elapse_finish,
                      expr_build_1->finish(&build_result_iterator_1));
  TIME_MICRO_OR_THROW(elapse_finish,
                      expr_probe->finish(&probe_result_iterator_base));
  auto probe_result_iterator =
      std::dynamic_pointer_cast<ResultIterator<arrow::RecordBatch>>(
