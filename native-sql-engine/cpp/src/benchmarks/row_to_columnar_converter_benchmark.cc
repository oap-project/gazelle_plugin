...
class BenchmarkRowToColumnar : public ::testing::Test {
 public:
...
TEST_F(BenchmarkRowToColumnar, test) {
  int64_t elapse_read = 0;
  int64_t elapse_init = 0;
  int64_t elapse_write = 0;
  int64_t row_to_columnar_init = 0;
  ...
  if (record_batch) {
    ...
    uint8_t* address = unsafe_row_writer_reader->GetBufferAddress();
    auto in_row_length = unsafe_row_writer_reader->GetLengths();
    std::shared_ptr<RowToColumnarConverter> row_to_columnar_converter =
      std::make_shared<RowToColumnarConverter>(schema, num_cols, num_rows,
                                               in_row_length, address, arrow::default_memory_pool());
    std::shared_ptr<arrow::RecordBatch> rb;
    TIME_NANO_OR_THROW(row_to_columnar_init, row_to_columnar_converter->Init(&rb));
    std::cout << "Took " << TIME_NANO_TO_STRING(row_to_columnar_init) << " to convert unsafeRows to record batch"
              << std::endl;
  }
}
...
int main(int argc, char** argv) {
  ...
  if (argc > 1) {
    for (int i = 1; i < argc; ++i) {
      sparkcolumnarplugin::rowtocolumnar::input_files.emplace_back(argv[i]);
    }
  }
