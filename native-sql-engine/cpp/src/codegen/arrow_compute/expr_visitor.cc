...
arrow::Status ExprVisitor::MakeWindow(
  ...
  *out = std::make_shared<ExprVisitor>(arrow::compute::ExecContext(memory_pool),
  ...
  for (const auto& child : node.children()) {
    ...
    if (child_func_name == "sum" || child_func_name == "avg" || child_func_name == "min" || child_func_name == "max" ||
        child_func_name == "count" || child_func_name == "count_literal" || child_func_name == "rank_asc" ||
