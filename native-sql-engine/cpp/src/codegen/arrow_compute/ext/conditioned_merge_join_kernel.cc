...
class ConditionedMergeJoinKernel::Impl {
 public:
  ...
  arrow::Status DoCodeGen(
    ...
    std::vector<gandiva::FieldVector> field_list = {left_field_list_,
                                                    right_field_list_};

    codegen_ctx->header_codes.push_back(
        R"(#include "codegen/common/sort_relation.h")");
    int idx = 0;
    for (auto relation_id : relation_id_) {
      auto relation_list_name =
          "sort_relation_" + std::to_string(relation_id) + "_";
      sort_prepare_ss
          << "auto typed_dependent_iter_list_" << relation_id
          << " = "
             "std::dynamic_pointer_cast<ResultIterator<SortRelation>>("
             "dependent_iter_list["
          << relation_id << "]);" << std::endl;
      sort_prepare_ss << "RETURN_NOT_OK(typed_dependent_iter_list_"
                      << relation_id << "->Next("
      ...
      for (int i = 0; i < field_list[idx].size(); i++) {
        ...
        sort_define_ss << "std::shared_ptr<"
                       << GetTemplateString(relation_col_type,
                                            "TypedRelationColumn", "Type",
                                            "arrow::")
                       << "> " << relation_col_name << ";" << std::endl;
        sort_define_ss << "bool " << relation_col_name << "_has_null;"
                       << std::endl;
        sort_prepare_ss << "RETURN_NOT_OK(" << relation_list_name
                        << "->GetColumn(" << i << ", &" << relation_col_name
                        << "));" << std::endl;
        sort_prepare_ss << relation_col_name
                        << "_has_null = " << relation_col_name << "->HasNull();"
                        << std::endl;
      }
      idx++;
    }
    ...
    for (int i = 0; i < 2; i++) {
      for (auto expr : project_codegen_list[i]) {
        ...
        project_output_list[i].push_back(
            std::make_pair(key_name, validity_name));
    ...
    for (auto pair : project_output_list[0])
      output_value_list.push_back(pair.first);
    auto left_paramater = GetParameterList(output_value_list, false);
    output_value_list.clear();
    for (auto pair : project_output_list[0])
      output_value_list.push_back(pair.second);
    auto left_validity_paramater =
        GetParameterList(output_value_list, false, " && ");
    output_value_list.clear();
    for (auto pair : project_output_list[1])
      output_value_list.push_back(pair.first);
    auto right_paramater = GetParameterList(output_value_list, false);
    output_value_list.clear();
    for (auto pair : project_output_list[1])
      output_value_list.push_back(pair.second);
    auto right_validity_paramater =
        GetParameterList(output_value_list, false, " && ");
    ...
    if (project_output_list[0].size() > 1) {
      function_define_ss << "auto left_tuple = std::forward_as_tuple("
                         << left_paramater << " );" << std::endl;
      left_tuple_name = "left_tuple";
    }
    if (project_output_list[1].size() > 1) {
      function_define_ss << "auto right_tuple = std::forward_as_tuple("
                         << right_paramater << " );" << std::endl;
      right_tuple_name = "right_tuple";
    }
    function_define_ss << "return " << left_tuple_name
                       << " == " << right_tuple_name << " ? 0 : ("
                       << left_tuple_name << " < " << right_tuple_name
                       << " ? -1 : 1);" << std::endl;
 ...
 private:
  ...
  arrow::Status GetInnerJoin(bool cond_check, bool use_relation_for_stream,
    ...
    auto streamed_relation =
        "sort_relation_" + std::to_string(relation_id_[1]) + "_";
    ...
    codes_ss << "if ((should_stop_ = !" << build_relation
             << "->NextNewKey())) break;" << std::endl;
    codes_ss << function_name << "_res = " << function_name << "();"
             << std::endl;
    codes_ss << "}" << std::endl;
    ...
    codes_ss << "for (int " << range_id << " = 0; " << range_id << " < "
             << range_name << "; " << range_id << "++) {" << std::endl;
    codes_ss << left_index_name << " = " << build_relation
             << "->GetItemIndexWithShift(" << range_id << ");" << std::endl;
    if (!cache_right) {
      codes_ss << "auto is_smj_" << relation_id << " = false;" << std::endl;
      codes_ss << right_for_loop_codes.str();
    }
    ...
    if (use_relation_for_stream) {
      finish_codes_ss << "} // end of Inner Join" << std::endl;
      finish_codes_ss << "should_stop_ = !" << streamed_relation
                      << "->NextNewKey();" << std::endl;
    }
  ...
  arrow::Status GetOuterJoin(bool cond_check, bool use_relation_for_stream,
    ...
    codes_ss << "auto " << function_name << "_res = " << function_name << "();"
             << std::endl;
    codes_ss << "while (" << function_name << "_res < 0) {" << std::endl;
    codes_ss << "if ((!" << build_relation << "->NextNewKey())) break;"
             << std::endl;
    codes_ss << function_name << "_res = " << function_name << "();"
             << std::endl;
    codes_ss << "}" << std::endl;
  ...
  arrow::Status GetAntiJoin(bool cond_check, bool use_relation_for_stream,
    ...
    if (cond_check) {
      ...
      codes_ss << range_name << " = " << build_relation
               << "->GetSameKeyRange();" << std::endl;
    } else {
      if (use_relation_for_stream) {
        codes_ss << "if ((should_stop_ = !" << streamed_relation
    ...
    if (use_relation_for_stream) {
      codes_ss << "auto " << streamed_range_name << " = " << streamed_relation
               << "->GetSameKeyRange();" << std::endl;
      codes_ss << "for (int " << streamed_range_id << " = 0; "
               << streamed_range_id << " < " << streamed_range_name << "; "
               << streamed_range_id << "++) {" << std::endl;
  ...
  arrow::Status GetExistenceJoin(bool cond_check, bool use_relation_for_stream,
    ...
    if (cond_check) {
      ...
      codes_ss << "}" << std::endl;
      codes_ss << "}" << std::endl;
      codes_ss << "auto " << exist_name << " = " << found_match_name << ";"
               << std::endl;
  ...
  arrow::Status GetProcessJoin(
    ...
    for (int idx = 0; idx < result_schema_index_list_.size(); idx++) {
      ...
      if (result_schema_index_list_[idx].first == 0) { /*left(streamed) table*/
        ...
        name = "sort_relation_" + std::to_string(relation_id_[0]) + "_" +
               std::to_string(i);
        type = left_field_list_[i]->type();
        arguments = left_index_name + ".array_id, " + left_index_name + ".id";
        if (join_type == 1) {
          valid_ss << output_validity << " = !" << fill_null_name << " && !("
                   << name << "_has_null && " << name << "->IsNull("
                   << arguments << "));" << std::endl;
        } else {
          valid_ss << output_validity << " = !(" << name << "_has_null && "
        ...
        define_ss << GetCTypeString(type) << " " << output_name << ";"
                  << std::endl;
      } else {                         /*right(streamed) table*/
        if (use_relation_for_stream) { /* use sort relation in streamed side*/
          if (exist_index_ != -1 && exist_index_ == i) {
            name = "sort_relation_" + std::to_string(relation_id_[0]) +
                   "_existence_value";
            type = arrow::boolean();
            valid_ss << output_validity << " = " << name << "_validity;"
                     << std::endl;
            ...
            arguments =
                right_index_name + ".array_id, " + right_index_name + ".id";
            valid_ss << output_validity << " = !(" << name << "_has_null && "
                     << name << "->IsNull(" << arguments << "));" << std::endl;
            valid_ss << "if (" << output_validity << ")" << std::endl;
            valid_ss << output_name << " = " << name << "->GetValue("
                     << arguments << ");" << std::endl;
          }
          right_output_idx_list.push_back(idx);
          define_ss << "bool " << output_name << "_validity = true;"
                    << std::endl;
          define_ss << GetCTypeString(type) << " " << output_name << ";"
                    << std::endl;
    ...
    if (left_output_idx_list.size() > right_output_idx_list.size())
      cache_right = false;

    switch (join_type) {
      case 0: { /* inner join */
        RETURN_NOT_OK(GetInnerJoin(cond_check, use_relation_for_stream,
                                   cache_right, output));
      } break;
      case 1: { /* Outer join */
        RETURN_NOT_OK(GetOuterJoin(cond_check, use_relation_for_stream,
                                   cache_right, output));
      } break;
      case 2: { /* Anti join */
      ...
      case 4: { /* Existence join */
        RETURN_NOT_OK(
            GetExistenceJoin(cond_check, use_relation_for_stream, output));
      } break;
      default: {
      } break;
    }
...
arrow::Status ConditionedMergeJoinKernel::Make(
  ...
  *out = std::make_shared<ConditionedMergeJoinKernel>(
      ...
      condition, join_type, result_schema, hash_relation_idx);
  return arrow::Status::OK();
}
...
std::string ConditionedMergeJoinKernel::GetSignature() {
