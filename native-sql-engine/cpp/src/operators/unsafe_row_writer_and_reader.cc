...
namespace unsaferow {
  ...
  int32_t GetVariableColsNum(std::shared_ptr<arrow::Schema> schema, int64_t num_cols) {
    ...
    for (auto i = 0; i < num_cols; i ++) {
      ...
      if ((type_id == arrow::BinaryType::type_id) || (type_id == arrow::StringType::type_id) || 
          (type_id == arrow::Decimal128Type::type_id)) count++;
    }
    return count;
  }
  ...
  arrow::Status  UnsafeRowWriterAndReader::Next(int64_t* length, 
      std::shared_ptr<arrow::ResizableBuffer>* buffer) {
  ...
  arrow::Status WriteValue(std::shared_ptr<arrow::ResizableBuffer> buffer, 
     int64_t offset, int32_t row_index, std::shared_ptr<arrow::Array> array, int64_t currentCursor, int32_t col_index,
