...
TimestampBuilder::TimestampBuilder(std::shared_ptr<arrow::DataType> type,
                                   arrow::MemoryPool* pool) {
  impl_ = std::make_shared<Impl>(type, pool);
}
arrow::Status TimestampBuilder::Append(const int64_t& value) {
...
arrow::Status TimestampBuilder::AppendNulls(int64_t length) {
  return impl_->AppendNulls(length);
}
arrow::Status TimestampBuilder::Finish(std::shared_ptr<arrow::Array>* out) {
  return impl_->Finish(out);
}
