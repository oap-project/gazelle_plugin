...
#define TYPED_SPARSE_HASH_MAP_IMPL(TYPENAME, TYPE)                            \
  ...
  arrow::Status TYPENAME::GetOrInsert(                                        \
      const TYPE& value, void (*on_found)(int32_t),                           \
...
#define TYPED_ARROW_HASH_MAP_DECIMAL_IMPL(HASHMAPNAME, TYPENAME, TYPE,      \
                                          MEMOTABLETYPE)                    \
  using MEMOTABLETYPE =                                                     \
      typename arrow::internal::HashTraits<arrow::TYPENAME>::MemoTableType; \
  class HASHMAPNAME::Impl : public MEMOTABLETYPE {                          \
  ...
  arrow::Status HASHMAPNAME::GetOrInsert(                                   \
      const TYPE& value, void (*on_found)(int32_t),                         \
      void (*on_not_found)(int32_t), int32_t* out_memo_index) {             \
    return impl_->GetOrInsert(&value, 16, on_found, on_not_found,           \
                              out_memo_index);                              \
  }                                                                         \
  int32_t HASHMAPNAME::GetOrInsertNull(void (*on_found)(int32_t),           \
                                       void (*on_not_found)(int32_t)) {     \
  ...
  int32_t HASHMAPNAME::Get(const TYPE& value) {                             \
    return impl_->Get(&value, 16);                                          \
  }                                                                         \
  int32_t HASHMAPNAME::GetNull() { return impl_->GetNull(); }
...
TYPED_ARROW_HASH_MAP_IMPL(UInt32HashMap, UInt32Type, uint32_t,
                          UInt32MemoTableType)
TYPED_ARROW_HASH_MAP_IMPL(UInt64HashMap, UInt64Type, uint64_t,
                          UInt64MemoTableType)
TYPED_ARROW_HASH_MAP_IMPL(FloatHashMap, FloatType, float, FloatMemoTableType)
TYPED_ARROW_HASH_MAP_IMPL(DoubleHashMap, DoubleType, double,
                          DoubleMemoTableType)
TYPED_ARROW_HASH_MAP_IMPL(Date32HashMap, Date32Type, int32_t,
                          Date32MemoTableType)
TYPED_ARROW_HASH_MAP_IMPL(Date64HashMap, Date64Type, int64_t,
                          Date64MemoTableType)
TYPED_ARROW_HASH_MAP_IMPL(StringHashMap, StringType, arrow::util::string_view,
                          StringMemoTableType)
TYPED_ARROW_HASH_MAP_DECIMAL_IMPL(Decimal128HashMap, Decimal128Type,
                                  arrow::Decimal128, DecimalMemoTableType)
