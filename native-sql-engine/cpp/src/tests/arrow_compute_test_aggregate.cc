...
TEST(TestArrowCompute, AggregateTest) {
  ...
  auto n_count =
      TreeExprBuilder::MakeFunction("action_count", {arg_0}, int64());
  auto n_sum_count =
      TreeExprBuilder::MakeFunction("action_sum_count", {arg_0}, int64());
  ...
  auto n_stddev = TreeExprBuilder::MakeFunction(
      "action_stddev_samp_final", {arg_2, arg_3, arg_4}, float64());
  ...
  auto n_action =
      TreeExprBuilder::MakeFunction("aggregateActions",
                                    {n_sum, n_count, n_sum_count, n_avg, n_min,
                                     n_max, n_stddev, n_count_literal},
                                    uint32());
  auto n_result = TreeExprBuilder::MakeFunction(
      "resultSchema",
      {TreeExprBuilder::MakeField(f_sum), TreeExprBuilder::MakeField(f_count),
       TreeExprBuilder::MakeField(f_sum_count),
       TreeExprBuilder::MakeField(f_count), TreeExprBuilder::MakeField(f_avg),
  ...
  std::vector<std::shared_ptr<Field>> ret_types = {
      f_sum, f_count, f_sum,    f_count,        f_avg,
      f_min, f_max,   f_stddev, f_count_literal};
  ///////////////////// Calculation //////////////////
  std::shared_ptr<CodeGenerator> expr;
  arrow::compute::ExecContext ctx;
  ASSERT_NOT_OK(CreateCodeGenerator(ctx.memory_pool(), sch, {aggr_expr},
                                    ret_types, &expr, true));
...
TEST(TestArrowCompute, GroupByCountAll) {
  ...
  auto n_groupby =
      TreeExprBuilder::MakeFunction("action_groupby", {arg0}, uint32());
  auto n_count =
      TreeExprBuilder::MakeFunction("action_countLiteral_1", {arg0}, uint32());
  auto n_proj =
      TreeExprBuilder::MakeFunction("aggregateExpressions", {arg0}, uint32());
  auto n_action = TreeExprBuilder::MakeFunction("aggregateActions",
                                                {n_groupby, n_count}, uint32());
  auto n_result =
      TreeExprBuilder::MakeFunction("resultSchema",
                                    {TreeExprBuilder::MakeField(f_unique),
                                     TreeExprBuilder::MakeField(f_count)},
                                    uint32());
  auto n_result_expr =
...
TEST(TestArrowCompute, GroupByHashAggregateWithCaseWhenTest) {
  ...
  auto n_when =
      TreeExprBuilder::MakeFunction("equal",
                                    {TreeExprBuilder::MakeField(f_0),
                                     TreeExprBuilder::MakeStringLiteral("BJ")},
                                    arrow::boolean());
  auto n_then =
      TreeExprBuilder::MakeFunction("multiply",
                                    {TreeExprBuilder::MakeField(f_1),
                                     TreeExprBuilder::MakeLiteral((double)0.3)},
                                    float64());
  auto n_else =
      TreeExprBuilder::MakeFunction("multiply",
                                    {TreeExprBuilder::MakeField(f_1),
                                     TreeExprBuilder::MakeLiteral((double)1.3)},
                                    float64());
  auto n_projection =
      TreeExprBuilder::MakeIf(n_when, n_then, n_else, float64());

  auto n_sum =
      TreeExprBuilder::MakeFunction("action_sum", {n_projection}, uint32());

  auto n_proj = TreeExprBuilder::MakeFunction(
      "aggregateExpressions",
      {TreeExprBuilder::MakeField(f_0), TreeExprBuilder::MakeField(f_1)},
      uint32());
  auto n_action = TreeExprBuilder::MakeFunction("aggregateActions",
                                                {n_groupby, n_sum}, uint32());
  auto n_result = TreeExprBuilder::MakeFunction(
      "resultSchema",
      {TreeExprBuilder::MakeField(f_unique), TreeExprBuilder::MakeField(f_sum)},
      uint32());
