...
template <class T, class IsNoexcept = std::integral_constant<
...
class weak_equality : public compare_internal::weak_equality_base<weak_equality> {
 ...
 public:
  ...
  // Comparisons
  friend constexpr bool operator==(weak_equality v,
...
class strong_equality : public compare_internal::strong_equality_base<strong_equality> {
 ...
 public:
  ...
  // Comparisons
  friend constexpr bool operator==(strong_equality v,
                                   compare_internal::OnlyLiteralZero<>) noexcept {
    return v.value_ == 0;
  }
  friend constexpr bool operator!=(strong_equality v,
...
class strong_ordering : public compare_internal::strong_ordering_base<strong_ordering> {
 ...
 public:
  ...
  constexpr operator weak_ordering() const noexcept {  // NOLINT
    return value_ == 0 ? weak_ordering::equivalent
...
template <
    ...
    phmap::enable_if_t<
        !std::is_same<bool, phmap::invoke_result_t<Compare, const K&, const LK&>>::value,
        int> = 0>
...
struct common_params {
  ...
  using node_count_type = phmap::conditional_t<(kNodeValueSpace / sizeof(value_type) >
...
template <typename Key, typename Compare, typename Alloc, int TargetNodeSize, bool Multi>
struct set_params
    : common_params<Key, Compare, Alloc, TargetNodeSize, Multi, set_slot_policy<Key>> {
  using value_type = Key;
...
class btree_node {
 ...
 public:
  ...
  field_type max_count() const {
    ...
    return max_cnt == field_type{kInternalNodeMaxCount} ? field_type{kNodeValues}
                                                        : max_cnt;
  }
  ...
  template <typename K>
  SearchResult<int, is_key_compare_to::value> lower_bound(const K& k,
  ...
  SearchResult<int, btree_is_key_compare_to<Compare, key_type>::value> linear_search(
      const K& k, const Compare& comp) const {
  ...
  template <typename K, typename Compare>
  SearchResult<int, false> linear_search_impl(const K& k, int s, const int e,
                                              const Compare& comp,
  ...
  template <typename K, typename Compare>
  SearchResult<int, true> linear_search_impl(const K& k, int s, const int e,
                                             const Compare& comp,
  ...
  template <typename K, typename Compare>
  SearchResult<int, false> binary_search_impl(const K& k, int s, int e,
                                              const Compare& comp,
                                              std::false_type /* IsCompareTo */) const {
    while (s != e) {
      const int mid = (s + e) >> 1;
  ...
  SearchResult<int, true> binary_search_impl(const K& k, int s, int e,
  ...
  static btree_node* init_internal(btree_node* n, btree_node* parent) {
    ...
    phmap::priv::SanitizerPoisonMemoryRegion(&n->mutable_child(0),
 ...
 private:
  ...
  // value j in node x.
  void uninitialized_move_n(const size_type n, const size_type i, const size_type j,
                            btree_node* x, allocator_type* alloc) {
    phmap::priv::SanitizerUnpoisonMemoryRegion(x->slot(j), n * sizeof(slot_type));
    for (slot_type *src = slot(i), *end = src + n, *dest = x->slot(j); src != end;
         ++src, ++dest) {
...
class btree {
 ...
 public:
  ...
  // Requirement: `key` is never referenced after consuming `args`.
  template <typename... Args>
  std::pair<iterator, bool> insert_hint_unique(iterator position, const key_type& key,
                                               Args&&... args);
...
template <typename P>
template <typename... Args>
inline void btree_node<P>::emplace_value(const size_type i, allocator_type* alloc,
                                         Args&&... args) {
  assert(i <= count());
...
template <typename P>
inline void btree_node<P>::remove_values_ignore_children(int i, size_type to_erase,
...
template <typename P>
void btree_node<P>::rebalance_right_to_left(const int to_move, btree_node* right,
                                            allocator_type* alloc) {
...
template <typename P>
void btree_node<P>::rebalance_left_to_right(const int to_move, btree_node* right,
                                            allocator_type* alloc) {
  assert(parent() == right->parent());
  assert(position() + 1 == right->position());
  ...
  if (right->count() >= to_move) {
    ...
    right->uninitialized_move_n(to_move, right->count() - to_move, right->count(), right,
                                alloc);
    for (slot_type *src = right->slot(right->count() - to_move - 1),
                   *dest = right->slot(right->count() - 1), *end = right->slot(0);
         src >= end; --src, --dest) {
    ...
    uninitialized_move_n(uninitialized_remaining, count() - uninitialized_remaining,
...
void btree_node<P>::swap(btree_node* x, allocator_type* alloc) {
  ...
  // Swap the values.
  for (slot_type *a = smaller->slot(0), *b = larger->slot(0), *end = a + smaller->count();
       a != end; ++a, ++b) {
  ...
  larger->uninitialized_move_n(to_move, smaller->count(), smaller->count(), smaller,
                               alloc);
...
constexpr bool btree<P>::static_assert_validation() {
  ...
  static_assert(kNodeValues < (1 << (8 * sizeof(typename node_type::field_type))),
...
auto btree<P>::erase(iterator _begin, iterator _end) -> std::pair<size_type, iterator> {
  ...
  while (size_ > target_size) {
    if (_begin.node->leaf()) {
      ...
      _begin =
          erase_from_leaf_node(_begin, (std::min)(remaining_to_erase, remaining_in_node));
    } else {
      _begin = erase(_begin);
    }
  }
  ...
  if (node != root()) {
    if (node->position() > 0) {
      ...
      if (left->count() < kNodeValues) {
        ...
        int to_move =
            (kNodeValues - left->count()) / (1 + (insert_position < kNodeValues));
...
template <typename P>
template <typename K>
inline auto btree<P>::internal_locate_impl(const K& key,
                                           std::false_type /* IsCompareTo */) const
...
class btree_container {
 ...
 protected:
  ...
  template <class K>
  using key_arg =
      typename KeyArg<IsTransparent<typename Tree::key_compare>::value>::template type<
...
class btree_set_container : public btree_container<Tree> {
 ...
 public:
  ...
  iterator insert(const_iterator position, const value_type& x) {
    return this->tree_.insert_hint_unique(iterator(position), params_type::key(x), x)
        .first;
  }
  iterator insert(const_iterator position, value_type&& x) {
    return this->tree_
        .insert_hint_unique(iterator(position), params_type::key(x), std::move(x))
        .first;
  }
  ...
  iterator emplace_hint(const_iterator position, Args&&... args) {
    init_type v(std::forward<Args>(args)...);
    return this->tree_
        .insert_hint_unique(iterator(position), params_type::key(v), std::move(v))
        .first;
  }
  ...
  insert_return_type insert(node_type&& node) {
    ...
    std::pair<iterator, bool> res = this->tree_.insert_unique(
        params_type::key(CommonAccess::GetSlot(node)), CommonAccess::GetSlot(node));
    if (res.second) {
...
class btree_map_container : public btree_set_container<Tree> {
 ...
 public:
  ...
  std::pair<iterator, bool> try_emplace(const key_type& k, Args&&... args) {
    return this->tree_.insert_unique(k, std::piecewise_construct,
  ...
  std::pair<iterator, bool> try_emplace(key_type&& k, Args&&... args) {
    ...
    return this->tree_.insert_unique(key_ref, std::piecewise_construct,
...
class btree_multiset_container : public btree_container<Tree> {
 ...
 public:
  ...
  template <typename... Args>
  iterator emplace_hint(const_iterator position, Args&&... args) {
    return this->tree_.insert_hint_multi(iterator(position),
                                         init_type(std::forward<Args>(args)...));
  }
  iterator insert(node_type&& node) {
    if (!node) return this->end();
    iterator res = this->tree_.insert_multi(params_type::key(CommonAccess::GetSlot(node)),
                                            CommonAccess::GetSlot(node));
    CommonAccess::Destroy(&node);
    return res;
  }
  iterator insert(const_iterator hint, node_type&& node) {
    ...
    iterator res = this->tree_.insert_hint_multi(
        iterator(hint), std::move(params_type::element(CommonAccess::GetSlot(node))));
    CommonAccess::Destroy(&node);
    return res;
  }
...
class btree_set : public priv::btree_set_container<priv::btree<priv::set_params<
