...
#pragma warning(disable : 4868)  // compiler may not enforce left-to-right evaluation
...
struct IsDecomposable<
    phmap::void_t<decltype(Policy::apply(
        RequireUsableKey<typename Policy::key_type, Hash, Eq>(), std::declval<Ts>()...))>,
...
void ConstructFromTupleImpl(Alloc* alloc, T* ptr, Tuple&& t,
  ...
  phmap::allocator_traits<Alloc>::construct(*alloc, ptr,
...
class raw_hash_set {
 ...
 private:
  ...
  using SlotAlloc =
      typename phmap::allocator_traits<allocator_type>::template rebind_alloc<slot_type>;
  using SlotAllocTraits =
      typename phmap::allocator_traits<allocator_type>::template rebind_traits<slot_type>;
  ...
  using RequiresInsertable =
      typename std::enable_if<phmap::disjunction<std::is_convertible<T, init_type>,
 ...
 public:
  ...
  class iterator {
   ...
   public:
    ...
    using reference = phmap::conditional_t<PolicyTraits::constant_iterators::value,
  ...
  template <class InputIter>
  raw_hash_set(InputIter first, InputIter last, size_t bucket_cnt, const hasher& hashfn,
  ...
  template <class T, RequiresNotInit<T> = 0, RequiresInsertable<T> = 0>
  raw_hash_set(std::initializer_list<T> init, size_t bucket_cnt, const hasher& hashfn,
               const allocator_type& alloc)
  ...
  raw_hash_set(const raw_hash_set& that)
      : raw_hash_set(
            that, AllocTraits::select_on_container_copy_construction(that.alloc_ref())) {}
  ...
  raw_hash_set& operator=(const raw_hash_set& that) {
    raw_hash_set tmp(that, AllocTraits::propagate_on_container_copy_assignment::value
                               ? that.alloc_ref()
                               : alloc_ref());
    swap(tmp);
    return *this;
  ...
  raw_hash_set& operator=(raw_hash_set&& that) noexcept(
    ...
    return move_assign(std::move(that),
  ...
  template <class InputIt,
            typename phmap::enable_if_t<has_difference_operator<InputIt>::value, int> = 0>
  void insert(InputIt first, InputIt last) {
    this->reserve(this->size() + (last - first));
    for (; first != last; ++first) emplace(*first);
  }

  template <class InputIt, typename phmap::enable_if_t<
  ...
  insert_return_type insert(node_type&& node) {
    ...
    auto res = PolicyTraits::apply(
        InsertSlot<false>{*this, std::move(*CommonAccess::GetSlot(node))}, elem);
    if (res.second) {
  ...
  insert_return_type insert(node_type&& node, size_t hashval) {
    ...
    const auto& elem = PolicyTraits::element(CommonAccess::GetSlot(node));
    auto res = PolicyTraits::apply(
        InsertSlotWithHash<false>{*this, std::move(*CommonAccess::GetSlot(node)),
                                  hashval},
        elem);
    if (res.second) {
      CommonAccess::Reset(&node);
  ...
  std::pair<iterator, bool> emplace_with_hash(size_t hashval, Args&&... args) {
    ...
    PolicyTraits::construct(&alloc_ref(), slot, std::forward<Args>(args)...);
    const auto& elem = PolicyTraits::element(slot);
    return PolicyTraits::apply(InsertSlotWithHash<true>{*this, std::move(*slot), hashval},
                               elem);
  }
 ...
 private:
    ...
    while (true) {
      ...
      for (int i : g.Match((h2_t)H2(hashval))) {
        ...
        if (PHMAP_PREDICT_TRUE(PolicyTraits::apply(
                EqualElement<K>{key, eq_ref()}, PolicyTraits::element(slots_ + offset))))
          return true;
      }
  ...
  void erase_meta_only(const_iterator it) {
    ...
    bool was_never_full =
        ...
        static_cast<size_t>(empty_after.TrailingZeros() + empty_before.LeadingZeros()) <
            Group::kWidth;
  ...
  void drop_deletes_without_resize() PHMAP_ATTRIBUTE_NOINLINE {
    ...
    for (size_t i = 0; i != capacity_; ++i) {
      ...
      size_t hashval =
          PolicyTraits::apply(HashElement{hash_ref()}, PolicyTraits::element(slots_ + i));
      auto target = find_first_non_full(hashval);
      size_t new_i = target.offset;
 ...
 private:
  ...
  template <size_t N, template <class, class, class, class> class RefSet, class M,
            class P, class H, class E, class A>
  ...
  phmap::priv::CompressedTuple<size_t /* growth_left */, hasher, key_equal,
                               allocator_type>
...
class raw_hash_map : public raw_hash_set<Policy, Hash, Eq, Alloc> {
 ...
 public:
  ...
  template <class K = key_type, class... Args,
            typename std::enable_if<!std::is_convertible<K, const_iterator>::value,
                                    int>::type = 0,
            K* = nullptr>
...
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
template <size_t N, template <class, class, class, class> class RefSet, class Mtx_,
...
class parallel_hash_set {
 ...
 public:
  ...
  // --------------------------------------------------------------------
  template <class K, class... Args>
  std::pair<iterator, bool> emplace_decomposable_with_hash(const K& key, size_t hashval,
                                                           Args&&... args) {
    Inner& inner = sets_[subidx(hashval)];
    auto& set = inner.set_;
    typename Lockable::UniqueLock m(inner);
    return make_rv(&inner,
                   set.emplace_decomposable(key, hashval, std::forward<Args>(args)...));
  }
  ...
  std::pair<iterator, bool> emplace_with_hash(size_t hashval, Args&&... args) {
    ...
    auto& set = inner.set_;
    typename Lockable::UniqueLock m(inner);
    typename EmbeddedSet::template InsertSlotWithHash<true> f{inner, std::move(*slot),
                                                              hashval};
  ...
  iterator lazy_emplace_with_hash(size_t hashval, const key_arg<K>& key, F&& f) {
    ...
    auto& set = inner.set_;
    typename Lockable::UniqueLock m(inner);
    return make_iterator(&inner,
                         set.lazy_emplace_with_hash(key, hashval, std::forward<F>(f)));
  }
 ...
 protected:
  ...
  template <class K>
  std::tuple<Inner*, size_t, bool> find_or_prepare_insert(
      const K& key, typename Lockable::UniqueLock& mutexlock) {
 ...
 public:
  ...
  template <class K = key_type, class... Args,
            typename std::enable_if<!std::is_convertible<K, const_iterator>::value,
                                    int>::type = 0,
            K* = nullptr>
  std::pair<iterator, bool> try_emplace_with_hash(size_t hashval, key_arg<K>&& k,
                                                  Args&&... args) {
  ...
  template <class K = key_type, class... Args,
            typename std::enable_if<!std::is_convertible<K, const_iterator>::value,
                                    int>::type = 0>
  std::pair<iterator, bool> try_emplace_with_hash(size_t hashval, const key_arg<K>& k,
                                                  Args&&... args) {
    return try_emplace_impl_with_hash(hashval, k, std::forward<Args>(args)...);
  }

  template <class K = key_type, class... Args, K* = nullptr>
  iterator try_emplace_with_hash(size_t hashval, const_iterator, key_arg<K>&& k,
                                 Args&&... args) {
    return try_emplace_with_hash(hashval, std::forward<K>(k), std::forward<Args>(args)...)
        .first;
  }

  template <class K = key_type, class... Args>
  iterator try_emplace_with_hash(size_t hashval, const_iterator, const key_arg<K>& k,
                                 Args&&... args) {
    return try_emplace_with_hash(hashval, k, std::forward<Args>(args)...).first;
  }

  // if map contains key, lambda is called with the mapped value (under read lock
  // protection), and if_contains returns true. This is a const API and lambda should not
  // modify the value
  // -----------------------------------------------------------------------------------------
  template <class K = key_type, class F>
  bool if_contains(const key_arg<K>& key, F&& f) const {
    return const_cast<parallel_hash_map*>(this)
        ->template modify_if_impl<K, F, typename Lockable::SharedLock>(
            key, std::forward<F>(f));
  }

  // if map contains key, lambda is called with the mapped value without read lock
  // protection, and if_contains_unsafe returns true. This is a const API and lambda
  // should not modify the value This should be used only if we know that no other thread
  // may be mutating the map at the time.
  // -----------------------------------------------------------------------------------------
  template <class K = key_type, class F>
  bool if_contains_unsafe(const key_arg<K>& key, F&& f) const {
    return const_cast<parallel_hash_map*>(this)
        ->template modify_if_impl<K, F, LockableBaseImpl<phmap::NullMutex>::DoNothing>(
            key, std::forward<F>(f));
  }

  // if map contains key, lambda is called with the mapped value  (under write lock
  // protection), and modify_if returns true. This is a non-const API and lambda is
  // allowed to modify the mapped value
  // ----------------------------------------------------------------------------------------------------
  template <class K = key_type, class F>
  ...
  // if map contains key, lambda is called with the mapped value  (under write lock
  // protection). If the lambda returns true, the key is subsequently erased from the map
  // (the write lock is only released after erase). returns true if key was erased, false
  // otherwise.
  // ----------------------------------------------------------------------------------------------------
  template <class K = key_type, class F>
  ...
  // value-constructed with the provided arguments (if any), as with try_emplace. if map
  // already  contains key, then the lambda is called with the mapped value (under write
  // lock protection) and can update the mapped value. returns true if key was not already
...
auto DecomposePair(F&& f, Args&&... args) -> decltype(memory_internal::DecomposePairImpl(
    std::forward<F>(f), PairArgs(std::forward<Args>(args)...))) {
  return memory_internal::DecomposePairImpl(std::forward<F>(f),
...
class NodeHashMapPolicy
 ...
 public:
  ...
  static value_type* new_element(Allocator* alloc, Args&&... args) {
    using PairAlloc =
        typename phmap::allocator_traits<Allocator>::template rebind_alloc<value_type>;
  ...
  static size_t GetNumProbes(const Set& set, const typename Set::key_type& key) {
    ...
    while (true) {
      ...
      for (int i : g.Match(priv::H2(hashval))) {
        if (Traits::apply(
                typename Set::template EqualElement<typename Set::key_type>{key,
                                                                            set.eq_ref()},
...
class flat_hash_set : public phmap::priv::raw_hash_set<phmap::priv::FlatHashSetPolicy<T>,
                                                       Hash, Eq, Alloc> {
...
template <class K, class V, class Hash, class Eq,
          class Alloc>  // default values in phmap_fwd_decl.h
class flat_hash_map
    : public phmap::priv::raw_hash_map<phmap::priv::FlatHashMapPolicy<K, V>, Hash, Eq,
                                       Alloc> {
...
template <class T, class Hash, class Eq,
          class Alloc>  // default values in phmap_fwd_decl.h
class node_hash_set : public phmap::priv::raw_hash_set<phmap::priv::NodeHashSetPolicy<T>,
                                                       Hash, Eq, Alloc> {
...
template <class Key, class Value, class Hash, class Eq,
          class Alloc>  // default values in phmap_fwd_decl.h
class node_hash_map
    : public phmap::priv::raw_hash_map<phmap::priv::NodeHashMapPolicy<Key, Value>, Hash,
                                       Eq, Alloc> {
...
class parallel_flat_hash_set
    : public phmap::priv::parallel_hash_set<N, phmap::priv::raw_hash_set, Mtx_,
                                            phmap::priv::FlatHashSetPolicy<T>, Hash, Eq,
                                            Alloc> {
...
class parallel_flat_hash_map
    : public phmap::priv::parallel_hash_map<N, phmap::priv::raw_hash_set, Mtx_,
                                            phmap::priv::FlatHashMapPolicy<K, V>, Hash,
                                            Eq, Alloc> {
...
class parallel_node_hash_set
    : public phmap::priv::parallel_hash_set<N, phmap::priv::raw_hash_set, Mtx_,
                                            phmap::priv::NodeHashSetPolicy<T>, Hash, Eq,
                                            Alloc> {
