...
#define PHMAP_ATTRIBUTE_ALWAYS_INLINE
    ...
    (PHMAP_HAVE_ATTRIBUTE(noinline) || (defined(__GNUC__) && !defined(__clang__)))
...
#define PHMAP_ATTRIBUTE_NO_SANITIZE_SAFESTACK
    ...
    (defined(__GNUC__) && (__GNUC__ > 5 || (__GNUC__ == 4 && __GNUC_MINOR__ >= 9)) && \
     !defined(__clang__))
...
#elif (PHMAP_HAVE_ATTRIBUTE(section) || (defined(__GNUC__) && !defined(__clang__))) && \
...
#define PHMAP_ASSERT(expr) \
  (PHMAP_PREDICT_TRUE((expr)) ? (void)0 : [] { assert(false && #expr); }())  // NOLINT
#endif
