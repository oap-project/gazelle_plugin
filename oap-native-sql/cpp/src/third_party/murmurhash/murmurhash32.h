...
template <typename T>
using enable_if_not_string = typename std::enable_if<!is_string<T>::value, int32_t>::type;
...
template <typename T>
inline enable_if_not_string<T> hash32(T in, bool validity, int32_t seed) {
  return validity ? hash32(double_to_long_bits(static_cast<double>(in)), seed) : seed;
}

template <typename T>
inline enable_if_not_string<T> hash32(T in, bool validity) {
