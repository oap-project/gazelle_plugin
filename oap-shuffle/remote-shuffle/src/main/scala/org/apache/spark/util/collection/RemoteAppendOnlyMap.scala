...
/**
  ...
  */
@DeveloperApi
class RemoteAppendOnlyMap[K, V, C](
    createCombiner: V => C,
    mergeValue: (C, V) => C,
  ...
  override def iterator: Iterator[(K, C)] = {
    if (currentMap == null) {
      throw new IllegalStateException(
        "RemoteAppendOnlyMap.iterator is destructive and should only be called once.")
    }
  ...
  private def hashKey(kc: (K, C)): Int = RemoteAppendOnlyMap.hash(kc._1)
...
private[spark] object RemoteAppendOnlyMap {
