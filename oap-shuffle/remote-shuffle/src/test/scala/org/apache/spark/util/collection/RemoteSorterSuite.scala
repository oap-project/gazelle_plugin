...
class RemoteSorterSuite extends SparkFunSuite with LocalSparkContext {
  ...
  test("spilling with hash collisions") {
    ...
    resolver =
      SparkEnv.get.shuffleManager.shuffleBlockResolver.asInstanceOf[RemoteShuffleBlockResolver]
    ...
    val sorter = new RemoteSorter[String, String, ArrayBuffer[String]](
      context, resolver, Some(agg), None, None)
  ...
  test("spilling with many hash collisions") {
    ...
    val sorter =
      new RemoteSorter[FixedHashObject, Int, Int](context, resolver, Some(agg), None, None)
  ...
  test("spilling with hash collisions using the Int.MaxValue key") {
    ...
    val sorter =
      new RemoteSorter[Int, Int, ArrayBuffer[Int]](context, resolver, Some(agg), None, None)
    sorter.insertAll(
  ...
  private def emptyDataStream(conf: SparkConf) {
    ...
    val sorter = new RemoteSorter[Int, Int, Int](
      context, resolver, Some(agg), Some(new HashPartitioner(3)), Some(ord))
    assert(sorter.iterator.toSeq === Seq())
    sorter.stop()

    // Only aggregator
    val sorter2 = new RemoteSorter[Int, Int, Int](
      context, resolver, Some(agg), Some(new HashPartitioner(3)), None)
    assert(sorter2.iterator.toSeq === Seq())
    sorter2.stop()

    // Only ordering
    val sorter3 = new RemoteSorter[Int, Int, Int](
      context, resolver, None, Some(new HashPartitioner(3)), Some(ord))
    assert(sorter3.iterator.toSeq === Seq())
    sorter3.stop()

    // Neither aggregator nor ordering
    val sorter4 = new RemoteSorter[Int, Int, Int](
      context, resolver, None, Some(new HashPartitioner(3)), None)
  ...
  private def fewElementsPerPartition(conf: SparkConf) {
    ...
    // Both aggregator and ordering
    val sorter = new RemoteSorter[Int, Int, Int](
      context, resolver, Some(agg), Some(new HashPartitioner(7)), Some(ord))
    ...
    // Only aggregator
    val sorter2 = new RemoteSorter[Int, Int, Int](
      context, resolver, Some(agg), Some(new HashPartitioner(7)), None)
    ...
    // Only ordering
    val sorter3 = new RemoteSorter[Int, Int, Int](
      context, resolver, None, Some(new HashPartitioner(7)), Some(ord))
    ...
    // Neither aggregator nor ordering
    val sorter4 = new RemoteSorter[Int, Int, Int](
      context, resolver, None, Some(new HashPartitioner(7)), None)
  ...
  private def cleanupIntermediateFilesInSorter(withFailures: Boolean): Unit = {
    ...
    assert(sorter.numSpills > 0, "sorter did not spill")
    assert(resolver.getAllFiles().nonEmpty, "sorter did not spill")
    sorter.stop()
    assert(resolver.getAllFiles().isEmpty, "spilled files were not cleaned up")
  }
  ...
  private def cleanupIntermediateFilesInShuffle(withFailures: Boolean): Unit = {
    ...
    assertSpilled(sc, "test shuffle cleanup") {
      if (withFailures) {
        ...
        // have been deleted.
        assert(resolver.getAllFiles().length === 2)
      } else {
        ...
        assert(resolver.getAllFiles().length === 4)
      }
    }
  }
  ...
  private def sortWithoutBreakingSortingContracts(conf: SparkConf) {
    ...
    val context = MemoryTestingUtils.fakeTaskContext(sc.env)
    val sorter1 = new RemoteSorter[String, String, String](
      context, resolver, None, None, Some(wrongOrdering))
    ...
    // Using aggregation and external spill to make sure RemoteSorter using
    ...
    val sorter2 = new RemoteSorter[String, String, ArrayBuffer[String]](
      context, resolver, Some(agg), None, None)
