...
case class StringSplitPart(str: Expression, regex: Expression, limit: Expression, index: Expression)
  ...
  def this(exp: Expression, regex: Expression, limit: Expression) = this(exp, regex, limit, Literal(0));
  ...
  override def prettyName: String = "splitpart"

  override protected def withNewChildrenInternal(
      newFirst: Expression, newSecond: Expression, newThird: Expression): StringSplitPart =
    copy(str = newFirst, regex = newSecond, limit = newThird)
}
