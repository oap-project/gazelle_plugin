...
private[oap] trait DataFiberBuilder {
...
private[oap] case class FixedSizeTypeFiberBuilder(
...
private[oap] case class BinaryFiberBuilder(
    defaultRowGroupRowCount: Int,
...
private[oap] case class StringFiberBuilder(
    defaultRowGroupRowCount: Int,
    ordinal: Int) extends DataFiberBuilder {
...
object DataFiberBuilder {
  def apply(dataType: DataType, ordinal: Int, defaultRowGroupRowCount: Int): DataFiberBuilder = {
    ...
    val dictionaryEnabled =
      System.getProperty("oap.encoding.dictionaryEnabled", "false").toBoolean
