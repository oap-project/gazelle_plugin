...
private[oap] case class ParquetFiberDataLoader(
    configuration: Configuration, reader: ParquetFiberDataReader, blockId: Int) extends Logging {
  ...
  def loadSingleColumn: FiberCache = {
    ...
    ParquetDataFiberWriter.dumpToCache(vector.asInstanceOf[OnHeapColumnVector], rowCount)
  }
}
