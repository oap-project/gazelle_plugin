...
class VectorizedCacheReader(
  ...
  protected def nextBatchInternal(): Unit = {
    columnarBatch.reset()
    val num = Math.min(columnarBatch.capacity.toLong, totalCountLoadedSoFar - rowsReturned).toInt
    val start = System.nanoTime()

    for (i <- fiberReaders.indices) {
      if (fiberReaders(i) != null) {
        fiberReaders(i).readBatch(currentRowGroupRowsReturned, num, columnarBatch.column(i)
