...
private [oap] object BPlusTreeMultiColumnSearchSuite extends Serializable {
...
class BPlusTreeMultiColumnSearchSuite extends SparkFunSuite
  ...
  val meta = new DataSourceMeta(
    ...
    OapFileFormat.OAP_DATA_FILE_CLASSNAME, null)
  ...
  private def assertScanner(meta: DataSourceMeta,
    ...
    ic.getScanner match {
      case Some(scanner: BPlusTreeScanner) =>
        assert(scanner._init(BPlusTreeMultiColumnSearchSuite.root).toSet === expectedIds, "")
  ...
  // Oap supports "Or" in order to make full use of index, but with one limit:
