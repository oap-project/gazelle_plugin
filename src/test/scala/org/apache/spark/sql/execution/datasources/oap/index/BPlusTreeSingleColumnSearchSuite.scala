...
private[oap] class IntValues(values: Array[Long]) extends IndexNodeValue {
  override def length: Int = values.length
  override def apply(idx: Int): Long = values(idx)
}

private[oap] class NonLeafNode(
    keys: Array[Key],
...
private[oap] class LeafNode(keys: Array[Key], values: Array[IntValues], sibling: IndexNode)
    extends IndexNode {
  override def length: Int = keys.length
...
class BPlusTreeSingleColumnSearchSuite extends SparkFunSuite
  ...
  val meta = new DataSourceMeta(
    ...
    OapFileFormat.OAP_DATA_FILE_CLASSNAME,
    null)
